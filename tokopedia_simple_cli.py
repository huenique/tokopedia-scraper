...
Usage:
    python tokopedia_cli.py -k "smartph            # If still none, try CSS class based selectors
            if len(product_containers) == 0:
                product_containers = await page.locator('.prd_container-card, .css-bk6tzz, .css-5wh65g, [class*="product"]').all()
                
            # Extract data from new products (skip already processed ones) "iPhone"
...
class TokopediaScraper:
    ...
    def extract_store_id_from_url(self, url: str) -> str:
        ...
        if match:
            ...
            if store_slug not in ['p', 'discovery', 'help', 'about', 'careers', 'search']:
                return store_slug
    ...
    async def handle_page_with_scroll(self, context: PlaywrightCrawlingContext) -> None:
        ...
        if max_pages:
            ...
            print(f"   🎯 Target: ~{max_pages} pages ({max_scroll_attempts} scroll attempts)")
        else:
        ...
        while scroll_attempts < max_scroll_attempts:
            ...
            product_containers: list[Locator] = await page.locator('div[data-testid="divProductWrapper"]').all()
            
            if len(product_containers) == 0:
                # Try alternative selectors for different page types
                product_containers = await page.locator('a[data-testid="lnkProductContainer"]').all()
            
            # If still no containers, try search page selectors
            if len(product_containers) == 0:
                product_containers = await page.locator('[data-testid*="product"], [data-testid*="Product"]').all()
            ...
            new_products: int = 0
            for i, container in enumerate(product_containers[scraped_count:], start=scraped_count):
                try:
                    product_data: dict[str, Any] = await self.extract_product_from_container(container, page.url)
                    if product_data and product_data.get('Title'):
                        self.scraped_products.append(product_data)
                        new_products += 1
                        print(f"   ✓ {len(self.scraped_products)}: {product_data.get('Title', 'Unknown')[:50]}...")
            ...
            new_containers: list[Locator] = await page.locator('div[data-testid="divProductWrapper"]').all()
            
            if len(new_containers) == 0:
                new_containers = await page.locator('a[data-testid="lnkProductContainer"]').all()
    ...
    async def extract_product_from_container(self, container: Locator, base_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            title_selectors: list[str] = [
                ...
                'h3', 'h4', 'h5',  # Generic headings
                'a[title]',  # Links with title attribute
                '[class*="name"]', '[class*="title"]'  # Generic class patterns
            ]
            ...
            for selector in title_selectors:
                try:
                    ...
                    if await title_elem.count() > 0:
                        ...
                        if title_attr and title_attr.strip() and len(title_attr.strip()) > 5:
            ...
            if store_texts:
                if len(store_texts) >= 2:
                    ...
                    if any(loc in text for loc in ['Jakarta', 'Surabaya', 'Bandung', 'Kab.', 'Kota']):
                        store_location = text
                    else:
                        store_name = text
            ...
            store_url: str | None = f"https://www.tokopedia.com/{store_id}" if store_id else None
            ...
            product_data: dict[str, Any] = {
                ...
                'Store Name': self.translator.translate(store_name) if store_name else None,
                'Store ID': store_id if store_id else None,
                'Store URL': store_url,
                'Product URL': product_url,
                'Image URL': image_url,
                'Brand': self.config.get('brand', ''),
                'Location': self.translator.translate(store_location) if store_location else None,
    ...
    async def run_scraper(self) -> list[dict[str, Any]]:
        ...
        print(f"🔍 Search Query: {self.config.get('brand', '')} {self.config['keyword']}".strip())
        ...
        try:
            ...
            if "status code: 410" in str(e) or "status code: 404" in str(e):
                ...
                search_query: str = f"{self.config.get('brand', '')} {self.config['keyword']}".strip()
                encoded_query: str = quote(search_query)
                fallback_url: str = f"https://www.tokopedia.com/search?st=product&q={encoded_query}"
